...
- **addon 库**（按需追加）：`libthingboot_addon_net_<模块>_<芯片>.a`，模块为 `ether` / `gsm` / `ble`
...
2. 在 `device.setup()` 之前调用对应安装函数注册驱动：

```cpp
tb_addon_net_ether_install();  // 以太网（W5500）
tb_addon_net_gsm_install();    // GSM/4G（ML307）
tb_addon_net_ble_install();    // BLE 配网 + 控制
```
