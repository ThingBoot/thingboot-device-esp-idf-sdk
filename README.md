...
- **BLE 配网+控制**：wifi_provisioning BLE 配网（TBC-<设备ID>、PoP 激活码、tbd-config 配置接口）；NimBLE GATT 控制服务（order write/event notify/config read+write），Kconfig `CONFIG_TBD_BLE` 启用变体库
- **BLE 配网+控制**：wifi_provisioning BLE 配网（TBC-<设备ID> 服务名、PoP 激活码、tbd-config 配置接口）；NimBLE GATT 控制服务（order write/event notify/config read+write），Kconfig `CONFIG_TBD_BLE` 启用变体库
...
| 平台 | 状态 |
|------|------|
| ESP32 | ✅ 全部变体（默认 / 以太网 / GSM / BLE） |
| ESP32-S3 | ✅ 全部变体 |
| ESP32-C6 | ✅ 全部变体 |
| ESP32-C3 | ✅ 全部变体 |
| ESP32-S2 / C2（ESP8684）等 | 计划支持 |

预编译库按 **芯片 × 功能变体** 命名：`libthingboot_device_<芯片>[_变体].a`（如 `libthingboot_device_esp32c6_ble.a`），组件 CMake 按 `IDF_TARGET` 与 Kconfig 自动选库。
