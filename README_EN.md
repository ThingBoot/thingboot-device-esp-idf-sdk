...
2. Call the matching install function to register the driver before `device.setup()`:

```cpp
tb_addon_net_ether_install();  // Ethernet (W5500)
tb_addon_net_gsm_install();    // GSM/4G (ML307)
tb_addon_net_ble_install();    // BLE provisioning + control
```
